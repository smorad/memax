...
class DeltaProduct(GRAS):
    ...
    def __init__(self, hidden_size: int, recurrent_size: int, rank: int, key):
        ...
        self.K = nn.Linear(
            hidden_size, recurrent_size * rank, use_bias=False, key=keys[0]
        )
        self.Q = nn.Linear(hidden_size, recurrent_size, use_bias=False, key=keys[1])
        self.V = nn.Linear(
            hidden_size, recurrent_size * rank, use_bias=False, key=keys[2]
        )
...
def make_layer(hidden_size: int, key, **overrides):
    """Build DeltaProduct for a residual trunk.
...
def make_semigroup(recurrent_size: int, *, key=None, **overrides):
    """Build the DeltaProduct semigroup. ``recurrent_size`` is the matrix side length."""
    return DeltaProductSemigroup(recurrent_size, **overrides)
